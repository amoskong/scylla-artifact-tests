...
class ScyllaInstallGeneric(object):
    ...
    def run(self):
        ...
        if self.uuid:
            ...
            last_id = self.cvdb.get_last_id(self.uuid, self.repoid, version, table='housekeeping.repodownload', filter="and file_name like 'scylla-server%{}'".format(version))
        for pkg in pkgs:
        ...
        # check install
        if self.uuid:
            assert self.cvdb.check_new_record(self.uuid, self.repoid, version, last_id, table='housekeeping.repodownload', filter="and file_name like 'scylla-server%{}'".format(version))
